    ...
    def registerInitialState(self, gameState):
        ...
        for x in range(layout.width):
            for y in range(layout.height):
                ...
                if walls[x][y] is 'r' or walls[x][y] is 'b':#calculate distance for positions which are reachable for at least one team
                    walls[x][y] = False
...
#         state1 = gameState.getAgentState(self.index[0])
#         state2 = gameState.getAgentState(self.index[1])
#          
#         myPos1 = state1.getPosition()
#         myPos2 = state2.getPosition()
#         
#         if myPos1 != nearestPoint(myPos1) or myPos2 != nearestPoint(myPos2):
#             # We're halfway from one position to the next
#             return state1.getDirection()
#         else:
...
#     def getMirror(self, gameState, agentIndex):
#         num = gameState.getNumAgents()/2
#         if agentIndex>num:
#             return gameState.getAgentState(agentIndex-num)
#         else:
#             return gameState.getAgentState(agentIndex+num)
        
    def getTransport(self, gameState):
        return gameState.data.layout.transport
