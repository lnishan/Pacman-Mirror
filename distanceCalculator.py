...
def computeDistances(layout):    
        ...
        for key, value in layout.transport:
            distances[(key, value)] = 0
            distances[(value, key)] = 0

        allNodes = layout.walls.asList(False)
        allNodes.extend(layout.walls.asList('b'))
        allNodes.extend(layout.walls.asList('r'))
        ...
        for source in allNodes:
            ...
            if source in layout.transport:
                dist[layout.transport[source]]=-1
            ...
            while not queue.isEmpty():
                ...
                for direction in directions:
                    ...
                    if not layout.walls[nextPos[0]][nextPos[1]]:
                        adjacent.append(nextPos)
                        
                if node in layout.transport:
                    adjacent.append(layout.transport[node])
