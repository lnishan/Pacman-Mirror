...
class Actions:
    ...
    def getPossibleActions(state, agentIndex):
        ...
        for direction, vec in Actions._directionsAsList:
            ...
            if type(walls[next_x][next_y]) is bool:
                if not walls[next_x][next_y]: 
                    possible.append(direction)
            else:
                if walls[next_x][next_y] is 'r':
...
class GameStateData:
    ...
    def __init__( self, prevState = None ):
        ...
        if prevState != None:
            ...
            self._foodEaten = prevState._foodEaten
            #self._foodAdded = prevState._foodAdded
            self._capsuleEaten = prevState._capsuleEaten
            self._capsuleTimer = prevState._capsuleTimer.copy()
        else:
            self._capsuleTimer = {}
...
#         self._flagEaten = None
    ...
    def initialize( self, layout, numGhostAgents ):
        ...
        #    pacman = layout.agentPositions[i]
        #    ghost = layout.agentPositions[i+numGhosts]
        #    self.agentStates.append( AgentState( [Configuration( pacman[1], Directions.STOP), Configuration( ghost[1], Directions.STOP)], [pacman[0], ghost[0]]))
    ...
    def deepCopy( self ):
        ...
        state._capsuleTimer = self._capsuleTimer.copy()
        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    ...
                    self.state = self.state.generateSuccessor( agentIndex + self.state.getNumAgents()/2, action )
                    self.display.update( self.state.data )
                except Exception,data:
                    self.mute(agentIndex)
                    self._agentCrash(agentIndex)
...
#             capsules = self.state.data.capsules
#             food = self.state.data.food
#             layoutFood = self.state.data.layout.food
# 
# 
#             
#             self.state.data._foodAdded=None
...
#                             self.state.data._foodAdded=(x, y)
#             self.display.update( self.state.data )
#             self.state.data._foodAdded=None   
#             
#             self.state.data._capsuleAdded = None
#             _capsuleTimer = self.state.data._capsuleTimer
...
#                     self.state.data._capsuleAdded = position
#                     self.display.update( self.state.data )
# 
#             self.state.data._capsuleAdded = None
            #if _BOINC_ENABLED:
