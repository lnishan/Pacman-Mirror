...
TEAM_COLORS.append(formatColor(.9, .7, .7))
TEAM_COLORS.append(formatColor(.7, .7, .9))
...
class InfoPane:
    def __init__(self, layout, gridSize, redTeam, blueTeam):
        ...
        self.base = (layout.height/2) * gridSize
    ...
    def drawPane(self):
        self.scoreText = text( self.toScreen(self.width/4, 0    ), self.textColor, self._infoString(0,0), "Consolas", self.fontSize, "bold")
    ...
    def updateScore(self, score, timeleft):
        changeText(self.scoreText, self._infoString(score,timeleft/2))
...
class PacmanGraphics:
    ...
    def drawAgentObjects(self, state):
        ...
        for index, agent in enumerate(state.agentStates):
            if agent.isPacman:
                ...
                image = self.drawGhost(agent, index - len(state.agentStates)/2)
                self.agentImages.append( (agent, image) )
        refresh()
    ...
    def update(self, newState):
        ...
        if agentState.isPacman:
            ...
            self.moveGhost(agentState, agentIndex-len(newState.agentStates)/2, prevState, prevImage)
        ...
        if newState._foodAdded != None:
            for food in newState._foodAdded:
                self.addFood(food, self.food, newState.layout)
        if newState._capsuleAdded !=None:
            self.addCapsule(newState._capsuleAdded, self.capsules)
    ...
    def drawPacman(self, pacman, index):
        ...
        if self.capture:
            outlineColor = GHOST_COLORS[index]
    ...
    def drawWalls(self, wallMatrix):
        ...
        for xNum, x in enumerate(wallMatrix):
            ...
            for yNum, cell in enumerate(x):
                if self.capture and (yNum * 2) < wallMatrix.height: 
                    wallColor = TEAM_COLORS[0]
                if self.capture and (yNum * 2) >= wallMatrix.height: 
                    wallColor = TEAM_COLORS[1]
                
                if wallMatrix[xNum][yNum] is 'r':
                    wallColor = TEAM_COLORS[2]
                if wallMatrix[xNum][yNum] is 'b':
                    wallColor = TEAM_COLORS[3]
    ...
    def getFoodSize(self, x, y):
        if self.layout.food[x][y]==10:
            return 0.1
        elif self.layout.food[x][y]==50:
            return 0.2
    def drawFood(self, foodMatrix ):
        ...
        for xNum, x in enumerate(foodMatrix):
            ...
            for yNum, cell in enumerate(x):
                if self.capture and (yNum * 2) < foodMatrix.height: color = TEAM_COLORS[0]
                if self.capture and (yNum * 2) >= foodMatrix.height: color = TEAM_COLORS[1]											 
                if cell: # There's food here
                    food_size = self.getFoodSize(xNum, yNum)
    ...
    def addFood(self, pos, foodImages, layout):
        ...
        x, y = pos
        color = TEAM_COLORS[0]
        if (y * 2) >= layout.height:
            color = TEAM_COLORS[1]
        
        food_size = self.getFoodSize(x, y)
        screen = self.to_screen(pos)
        dot = circle(screen,
                            food_size * self.gridSize,
                            outlineColor = color,
    ...
    def addCapsule(self, pos, capsuleImages):
        x, y = pos
